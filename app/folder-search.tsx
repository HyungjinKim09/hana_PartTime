...
export function FolderSearch({folders,query,onQuery,view,path,onOpen}:{folders:Folder[];query:string;onQuery:(value:string)=>void;view:'date'|'address';path:string[];onOpen:(folder:Folder,path:string[],leaf:boolean)=>void}){
 const results=useMemo(()=>searchFolders(folders,query,view,path),[folders,query,view,path]);
 return <section className="folder-search" aria-label="폴더 검색">
  ...
  {query.trim()&&<><p role="status" aria-live="polite">검색 결과 {results.length}개 폴더</p><div className="folder-grid">{results.map(result=><button className="folder-card" key={JSON.stringify([result.folder.region,view==='date'?result.folder.date:'',result.path])} onClick={()=>onOpen(result.folder,result.path,result.leaf)}><div className="folder-card-top"><FolderOpen size={34}/><ChevronRight size={18}/></div><h3>{result.label}</h3><p>{result.folder.region}{view==='date'?' · '+result.folder.date:''}</p><p>{result.folder.address}</p>{!result.leaf&&<p>동·호수별 폴더 보기</p>}<div className="folder-card-bottom"><span><ImageIcon size={15}/>{result.count}장의 사진</span><span>폴더 열기</span></div></button>)}</div>{!results.length&&<div className="empty-photos"><Search size={30}/><h3>검색 결과가 없어요</h3><p>주소나 건물명의 일부로 검색하거나 상위 폴더에서 다시 검색해 주세요.</p></div>}</>}
 </section>;
}
