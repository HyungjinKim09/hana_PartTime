...
export async function GET(request:Request){try{
  ...
  const folders=(await listFolders(db,owner)).filter(f=>(!folder||f.id===folder)&&(!region||f.region===region)&&(!date||f.date===date));
  ...
  if(!folder){
    ...
    for(const source of sources.results.filter(s=>folders.some(f=>f.region===s.region&&f.date===s.survey_date)))entries.push({name:`${safeFilename(source.region)}/${source.survey_date}/일정표_${source.id}_${safeFilename(source.filename)}`,size:source.size,open:async()=>{const object=await bucket.get(source.object_key);if(!object)throw new Error('Missing schedule original');return object.body;}});
  }
  const name=safeFilename([region||'현장사진',date,folder?folders[0].lot:'전체'].filter(Boolean).join('_'))+'.zip';
