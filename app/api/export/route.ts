...
export async function GET(request:Request){try{
  ...
  if(!folder){
    ...
    for(const source of sources.results.filter(s=>folders.some(f=>f.region===s.region&&f.date===s.survey_date)))entries.push({name:`${source.survey_date}/일정표_${source.id}_${safeFilename(source.filename)}`,size:source.size,open:async()=>{const object=await bucket.get(source.object_key);if(!object)throw new Error('Missing schedule original');return object.body;}});
  }
