...
export async function GET(request:Request){try{
  ...
  for(const f of [...allFolders].sort((a,b)=>a.id.localeCompare(b.id))){
    ...
    const key=(value:string)=>`${safeFilename(f.region)}/${f.date}/${value}`.toLowerCase();
  ...
  for(const f of folders){const path=`${safeFilename(f.region)}/${f.date}/${folderNames.get(f.id)}/`;
