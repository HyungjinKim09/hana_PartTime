...
export default function Workspace({userName}:{userName:string}){
  ...
  async function upload(files:File[]){
    ...
    if(saved)toast.success(`${saved}장 저장 완료 · 사직동 ${target}`);
  ...
  return <div className="app-shell">
    ...
    <div className="workspace-layout">
      <aside className="project-panel"><div className="panel-label">조사 프로젝트</div><div className="project-title"><span className="project-icon"><FolderIcon size={21}/></span><div><strong>사직4구역</strong><span>현장조사 사진</span></div></div><div className="project-divider"/><button className="date-nav" disabled={uploading} onClick={()=>navigate(null)}><FolderOpen size={19}/><span>2026. 09. 17</span><span className="nav-count">12</span></button><div className="folder-index">{folders.map(f=><button key={f.id} className={selected===f.id?'index-item active':'index-item'} onClick={()=>navigate(f.id)} disabled={uploading}><FolderIcon size={15}/><span>{f.id}</span><span>{f.count}</span></button>)}</div><div className="sidebar-bottom"><LockKeyhole size={16}/><div><strong>비공개 원본 보관</strong><p>같은 계정으로 PC에서도<br/>사진을 확인할 수 있어요.</p></div></div></aside>
      <main className="main-workspace">
        <nav aria-label="현재 위치" className="breadcrumb"><button disabled={uploading} onClick={()=>navigate(null)}>사직4구역</button><ChevronRight size={14}/><button disabled={uploading} onClick={()=>navigate(null)}>2026.09.17</button>{active&&<><ChevronRight size={14}/><span>{active.id}</span></>}</nav>
        <section className="page-heading"><div><div className="eyebrow">FIELD SURVEY · PHOTO ARCHIVE</div><h1>{active?`사직동 ${active.id}`:'9월 17일 현장조사'}<span className="heading-label">{active?'번지 폴더':'목요일'}</span></h1><p>{active?active.address:'일정표의 번지별로 사진을 모으고, 폴더 그대로 내려받으세요.'}</p></div><button className="primary-button" onClick={()=>setExportScope(null)} disabled={loading||uploading||!!error}><Archive size={18}/>전체 다운로드</button></section>
        ...
        {!active&&!selected&&<><div className="overview-strip"><div><span>조사 대상</span><strong>12<small>곳</small></strong></div><div><span>사진 있는 폴더</span><strong>{folders.filter(f=>f.count>0).length}<small>/ 12</small></strong></div><div><span>보관한 사진</span><strong>{total}<small>장</small></strong></div><div className="storage-stat"><CloudUpload size={20}/><span>원본 저장<small>{formatBytes(bytes)}</small></span></div></div>
          <div className="section-heading"><h2><FolderOpen size={20}/>번지별 폴더 <span>12</span></h2><button className="subtle-button" onClick={()=>refresh(null)} disabled={loading}><RefreshCw size={15} className={loading?'spin':''}/>새로고침</button></div>
          {loading&&!folders.length?<div className="loading-block" role="status">일정과 사진 보관함을 불러오고 있어요.</div>:[1,2].map(group=><section className="schedule-group" key={group}><div className="group-label">일정표 {group===1?'상단':'하단'}<span>{group===1?'7':'5'}개 폴더</span></div><div className="folder-grid">{folders.filter(f=>f.group===group).map(f=><button className="folder-card" key={f.id} onClick={()=>navigate(f.id)}><div className="folder-card-top"><FolderIcon className="folder-symbol" size={42} strokeWidth={1.35}/><span className="time-pill"><Clock size={13}/>{f.time}</span></div><h3>사직동 {f.id}</h3><p>{f.address}</p><div className="folder-card-bottom"><span><ImageIcon size={15}/>{f.count?`${f.count}장의 사진`:'사진 없음'}</span>{f.warning?<span className="warning-label">2인 방문</span>:<ChevronRight size={17}/>}</div></button>)}</div></section>)}
        </>}
        {active&&<><button className="back-button" disabled={uploading} onClick={()=>navigate(null)}><ArrowLeft size={16}/>전체 폴더</button><section className="site-details"><div><span className="detail-label"><Clock size={14}/>방문 시간</span><strong>{active.time}</strong></div><div><span className="detail-label"><UserRound size={14}/>연락 대상</span><strong>{active.name}</strong><div className="phone-list">{active.phones.map(p=><a key={p} href={'tel:'+p}><Phone size={13}/>{p}</a>)}</div></div><div className="visit-note"><span className="detail-label"><TriangleAlert size={14}/>조사 메모</span><p>{active.notes}</p></div></section>
    ...
    <Dialog open={exportScope!==undefined} onOpenChange={open=>{if(!open)setExportScope(undefined);}}><DialogContent><DialogHeader><DialogTitle>폴더 그대로 다운로드</DialogTitle><DialogDescription>사진 원본을 번지별로 정리한 ZIP 파일로 받습니다.</DialogDescription></DialogHeader><div className="export-summary"><Archive size={30}/><div><strong>{exportFolder?`사직동 ${exportFolder.id}`:'사직4구역 전체 폴더'}</strong><p>{exportFolder?'1개':`${folders.length}개`} 폴더 · {exportFolder?exportFolder.count:total}장 · {formatBytes(exportFolder?exportFolder.bytes:bytes)}</p></div></div><div className="export-path"><FolderIcon size={17}/>사직4구역 / 2026-09-17 / 번지 / 사진</div><p className="export-help">빈 폴더도 포함합니다. 같은 이름의 사진은 고유번호를 붙여 모두 보존합니다. 다운로드 완료 여부는 브라우저 다운로드 목록에서 확인해 주세요.</p><a className="primary-button full-width" href={'/api/export'+(exportFolder?'?folder='+encodeURIComponent(exportFolder.id):'')} target="_blank" rel="noopener" onClick={()=>{setExportScope(undefined);toast.info('다운로드를 요청했어요. 브라우저 다운로드 목록을 확인해 주세요.');}}><ArrowDownToLine size={18}/>ZIP 다운로드</a></DialogContent></Dialog>
