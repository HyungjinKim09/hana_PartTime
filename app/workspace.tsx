...
export default function Workspace({userName,isAdmin=false}:{userName:string;isAdmin?:boolean}){
  ...
  useEffect(()=>{if(!uploading&&!remarksDirty)return;const warn=(e:BeforeUnloadEvent)=>{e.preventDefault();};window.addEventListener('beforeunload',warn);return()=>window.removeEventListener('beforeunload',warn);},[uploading,remarksDirty]);
  ...
  return <div className="app-shell">
    ...
    <Dialog open={exportScope!==undefined} onOpenChange={open=>{if(!open)setExportScope(undefined);}}><DialogContent><DialogHeader><DialogTitle>폴더 그대로 다운로드</DialogTitle><DialogDescription>사진 원본과 일정표를 날짜 · 일정별로 정리한 ZIP 파일로 받습니다.</DialogDescription></DialogHeader><div className="export-summary"><Archive size={30}/><div><strong>{(exportFolder?folderLabel(exportFolder):'')||[region,date].filter(Boolean).join(' / ')||'모든 지역과 날짜'}</strong><p>{exportItems.length}개 일정 폴더 · {exportItems.reduce((n,f)=>n+f.count,0)}장 · {formatBytes(exportItems.reduce((n,f)=>n+f.bytes,0))}</p></div></div><div className="export-path"><FolderIcon size={17}/>조사 날짜 / 호수 또는 지번 주소 / 사진</div><p className="export-help">구분건물은 호수만(예: 301호), 일반건물은 지번 주소로 폴더를 만듭니다. 이름이 겹치면 번호를 붙입니다. 빈 폴더도 포함합니다. 같은 이름의 사진은 고유번호를 붙여 모두 보존합니다. 다운로드 완료 여부는 브라우저 다운로드 목록에서 확인해 주세요.</p><a className="primary-button full-width" href={'/api/export?'+exportQuery.toString()} target="_blank" rel="noopener" onClick={()=>{setExportScope(undefined);toast.info('다운로드를 요청했어요. 브라우저 다운로드 목록을 확인해 주세요.');}}><ArrowDownToLine size={18}/>ZIP 다운로드</a></DialogContent></Dialog>
