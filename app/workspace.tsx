...
export default function Workspace({userName,isAdmin=false}:{userName:string;isAdmin?:boolean}){
  ...
  const scoped=baseFolders.filter(f=>withinAddress(f,addressPath));
  const nodes=addressNodes(baseFolders,addressPath);
  const pathLabels=addressPath.map((key,i)=>addressParts(scoped[0]||baseFolders.find(f=>withinAddress(f,addressPath.slice(0,i+1)))||{id:'',region:'',lot:'',address:''})[i]?.label||(key==='@whole'?'건물 전체·호수 미입력':'주소'));
  ...
  function location(r:string|null,d:string|null=null,id:string|null=null,path:string[]=[],mode=view){
    ...
    setView(mode);setAddressPath(path);setRegion(r);setDate(d);setSelected(id);setPreview(null);setExportScope(undefined);window.scrollTo({top:0,behavior:'smooth'});
  }
  ...
  return <div className="app-shell">
    ...
    <div className="workspace-layout">
      ...
      <main className="main-workspace">
        ...
        <div className="media-tabs" role="group" aria-label="폴더 보기 방식">{(['date','address'] as const).map(mode=><button key={mode} className={view===mode?'primary-button':'secondary-button'} aria-pressed={view===mode} disabled={uploading||remarksDirty||cameraPending} onClick={()=>location(region,null,null,[],mode)}>{mode==='date'?'구역 → 날짜 → 주소':'구역 → 주소'}</button>)}</div><section className="page-heading"><div><div className="eyebrow">FIELD SURVEY · PHOTO ARCHIVE</div><h1>{active?pathLabels.at(-1)||folderLabel(active):pathLabels.at(-1)|| (date?`${date} 현장조사`:region||'현장조사 보관함')}<span className="heading-label">{active?'사진 폴더':addressPath.length?'주소별':date?'주소별':region?(view==='address'?'주소별':'날짜별'):'지역별'}</span></h1><p>{active?active.address:'일정표 사진을 등록하고 지역 · 날짜 · 번지별로 현장 사진을 모으세요.'}</p></div><div className="heading-actions">{region&&(!addressPath.length||active)&&<button className="subtle-button" disabled={loading||uploading||remarksDirty||folderDeleteBusy||cameraPending} onClick={()=>setFolderDelete(true)}><Trash2 size={17}/>{active?'선택 일정 삭제':'폴더 삭제'}</button>}<button className="secondary-button" disabled={loading||uploading||remarksDirty||cameraPending} onClick={()=>{setManualImport(true);setImporting(true);}}><Plus size={18}/>일정 직접 추가</button>{region&&date&&<DailyReportButton region={region} date={date} disabled={loading||uploading||remarksDirty||!!error}/>}<button className="secondary-button" disabled={uploading||remarksDirty||cameraPending} onClick={()=>{setManualImport(false);setImporting(true);}}><CloudUpload size={18}/>일정표 사진 등록</button><button className="primary-button" onClick={()=>setExportScope(active?.id??null)} disabled={loading||uploading||!!error}><Archive size={18}/>{region?'현재 폴더 다운로드':'전체 다운로드'}</button></div></section>
