...
export function ArchiveDownload({query,name,onComplete}:{query:URLSearchParams;name:string;onComplete:()=>void}){
  ...
  async function start(part?:number){
    ...
    let last=0;
    try{
      const params=new URLSearchParams(query);if(part!==undefined&&parts){params.set('part',String(part));params.set('snapshot',parts.snapshot);}
