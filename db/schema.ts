...
export const surveyFolders=sqliteTable('survey_folders',{
  owner:text('owner').notNull(),id:text('id').notNull(),region:text('region').notNull(),date:text('survey_date').notNull(),lot:text('lot').notNull(),
...
},t=>[primaryKey({columns:[t.owner,t.id]}),uniqueIndex('survey_folders_owner_day_lot').on(t.owner,t.region,t.date,t.lot)]);
