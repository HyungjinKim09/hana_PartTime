...
test('R2 budgets enforce concurrent reservations, rolling windows and safe failure recovery',async t=>{
  ...
  try{
    ...
    await t.test('missing accounting tables block R2 access',async()=>{
      ...
      assert.equal((await budgetUsage(db)).reads,0,'unused tickets do not reserve reads');
      ...
      assert.equal((await budgetUsage(db)).reads,1,'only the redeemed ticket charges a read');
