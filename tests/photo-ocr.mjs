...
const source=await sharp(process.argv[2]).resize({width:2200,height:4000,fit:'inside'}).ensureAlpha().raw().toBuffer({resolveWithObject:true});
...
try{
 ...
 if(expectedIndex>=0){const expected=JSON.parse(await readFile(process.argv[expectedIndex+1],'utf8'));assert.equal(draft.region,expected.region);assert.equal(draft.date,expected.date);assert.deepEqual(result.folders.map(({lot,time,address})=>({lot,time,address})),expected.folders);console.log('PASS: supplied photograph matches all expected lots, times and road addresses.');}
