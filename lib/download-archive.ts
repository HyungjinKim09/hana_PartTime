...
export async function downloadArchive(query:URLSearchParams,signal:AbortSignal,onProgress:(bytes:number,total:number)=>void){
  ...
  const handle=picker?await picker.call(window,{suggestedName:'HANA-사진.zip',types:[{description:'ZIP 파일',accept:{'application/zip':['.zip']}}]}):null;
