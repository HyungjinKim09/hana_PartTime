...
function reportLot(lot:string){return lot.trim().replace(/(\d+)(?:\s*[-–−]\s*(\d+))?$/u,(_match,main:string,sub?:string)=>main.padStart(3,'0')+(sub?'-'+sub.padStart(3,'0'):''));}
export function exportFolderLabel(f:Pick<Folder,'unit'|'address'|'lot'>){
  if(buildingCategory(f)==='일반건물'){
    const lot=reportLot(f.lot);
...
export function dailyReport(region:string,date:string,folders:Folder[]){
  ...
  for(const {category,rows} of groups){
    ...
    rows.forEach((f,i)=>{
      ...
      const heading=`${i+1}.${reportLot(f.lot)}${place?`(${place})`:''}`;
  ...
  if(additional.length){
    ...
    additional.forEach((f,i)=>{
      ...
      lines.push(`${i+1}. ${reportLot(f.lot)}${place?`(${place})`:''} - ${surveyStatus(f.surveyStatus)} / ${detail?detail+' / ':''}${f.remarks?.trim()||'특이사항 없음'}`);
    });lines.push('');
  }
