...
export function searchFolders(folders:Folder[],query:string,view:'date'|'address',parentPath:string[]=[]){
 ...
 const groups=new Map<string,{folder:Folder;folders:Folder[];label:string;path:string[];count:number;matches:boolean;leaf:boolean}>();
 for(const f of folders){
  ...
  if(!group){group={folder:f,folders:[],label:parts.slice(0,depth).map(p=>p.label).join(' / '),path,count:0,matches:false,leaf:true};groups.set(key,group);}
