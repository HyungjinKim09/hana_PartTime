...
export function folderView(row:FolderRow):Folder{return {id:row.id,region:row.region,date:row.survey_date,lot:row.lot,unit:row.unit,time:row.time,name:row.name,phones:JSON.parse(row.phones),address:row.address,notes:row.notes,remarks:row.remarks,buildingDetails:row.building_details,surveyStatus:row.survey_status,group:row.group_index,warning:!!row.warning,count:row.count??0,bytes:row.bytes??0};}
